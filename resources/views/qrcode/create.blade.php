...
<div class="container mx-auto px-4 py-2 max-w-screen-2xl">
    <div class="bg-white/90 backdrop-blur-md p-4 rounded-xl shadow border border-white/20">
        <h1 class="text-xl font-bold text-gray-900 text-center mb-4">Tạo QR Code Chuyên Nghiệp</h1>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {{-- CỘT 1: Panel chọn chế độ + Tùy chỉnh --}}
            <div class="flex flex-col gap-4">
                {{-- Dòng 1: PANEL CHỌN LOẠI NỘI DUNG --}}
                <div class="p-2 bg-gray-50 rounded-lg border border-gray-200">
                    <div id="content-type-toolbar" class="flex gap-1 overflow-x-auto pb-1 -mx-1 px-1">
                        @foreach([
                            ['url','🌐','URL'],
                            ['text','📝','Văn bản'],
                            ['wifi','📶','WiFi'],
                            ...
                            <button type="button"
                                ...
                                data-type="{{$type}}">
                                <span class="w-4 h-4 inline-block">{{$icon}}</span><span>{{$label}}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
                {{-- Dòng 2: Tùy chỉnh nội dung + màu sắc + shape... --}}
                <div class="p-3 bg-gray-50 rounded-lg border border-gray-200 space-y-3">

                    {{-- Trường dữ liệu theo loại nội dung --}}
                    <div id="url-fields" class="content-fields">
                        <label for="url-input" class="block text-gray-700 font-semibold mb-1 text-sm">URL:</label>
                    ...
                    <div id="text-fields" class="content-fields hidden">
                        <label for="text-input" class="block text-gray-700 font-semibold mb-1 text-sm">Văn bản:</label>
                    ...
                    <div id="wifi-fields" class="content-fields hidden">
                        <label for="wifi-ssid" class="block text-gray-700 font-semibold mb-1 text-sm">Tên WiFi (SSID):</label>
                        <input type="text" id="wifi-ssid"
                            class="w-full p-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            placeholder="Tên mạng của bạn">
                        <label for="wifi-password" class="block text-gray-700 font-semibold mb-1 text-sm mt-1">Mật khẩu:</label>
                        <input type="text" id="wifi-password"
                            class="w-full p-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            placeholder="Mật khẩu WiFi">
                        <label for="wifi-encryption" class="block text-gray-700 font-semibold mb-1 text-sm mt-1">Loại bảo mật:</label>
                    ...
                    <div id="phone-fields" class="content-fields hidden">
                        <label for="phone-number" class="block text-gray-700 font-semibold mb-1 text-sm">Số điện thoại:</label>
                        <input type="tel" id="phone-number"
                            class="w-full p-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            placeholder="0901234567">
                    </div>

                    <div id="email-fields" class="content-fields hidden">
                        <label for="email-address" class="block text-gray-700 font-semibold mb-1 text-sm">Email:</label>
                        <input type="email" id="email-address"
                            class="w-full p-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            placeholder="contact@example.com">
                        <label for="email-subject" class="block text-gray-700 font-semibold mb-1 text-sm mt-1">Tiêu đề:</label>
                        <input type="text" id="email-subject"
                            class="w-full p-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            placeholder="Tiêu đề email">
                        <label for="email-body" class="block text-gray-700 font-semibold mb-1 text-sm mt-1">Nội dung:</label>
                        <textarea id="email-body" rows="2"
                            class="w-full p-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            placeholder="Nội dung email"></textarea>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-gray-700 font-semibold text-sm">Body Shape</label>
                            <input type="color" id="dots-color" value="#000000"
                                class="w-7 h-7 border rounded cursor-pointer">
                        </div>
                        <div id="dots-type-buttons" class="grid grid-cols-6 gap-1 mb-1">
                            <button type="button" class="dots-type-btn btn-chip" data-value="square">■</button>
                            <button type="button" class="dots-type-btn btn-chip" data-value="dots">●</button>
                            <button type="button" class="dots-type-btn btn-chip" data-value="rounded">◼︎</button>
                            <button type="button" class="dots-type-btn btn-chip" data-value="extra-rounded">▣</button>
                            <button type="button" class="dots-type-btn btn-chip" data-value="classy">◆</button>
                            <button type="button" class="dots-type-btn btn-chip" data-value="classy-rounded">◇</button>
                        </div>
                        <label class="flex items-center gap-2 mb-1 text-xs">
                            <input id="dots-grad-enabled" type="checkbox" class="h-3 w-3"> <span>Dùng gradient</span>
                        </label>
                        <div id="dots-gradient-options" class="gradient-options hidden">
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                <select id="dots-grad-type" class="p-1 border rounded text-xs">
                                    <option value="linear">Linear</option>
                                    <option value="radial">Radial</option>
                                </select>
                                <input type="color" id="dots-grad-c1" value="#000000" class="w-full h-7 border rounded">
                                <input type="color" id="dots-grad-c2" value="#555555" class="w-full h-7 border rounded">
                                <div class="col-span-2">
                                    <label class="text-xs text-gray-600">Góc (°)</label>
                                    <input id="dots-grad-rot" type="range" min="0" max="360" value="0" class="w-full">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-gray-700 font-semibold text-sm">Eye Frame</label>
                            <input type="color" id="corners-square-color" value="#000000"
                                class="w-7 h-7 border rounded cursor-pointer">
                        </div>
                        <div id="corners-square-type-buttons" class="grid grid-cols-4 gap-1 mb-1">
                            <button type="button" class="corners-square-type-btn btn-chip" data-value="square">▢</button>
                            <button type="button" class="corners-square-type-btn btn-chip"
                                data-value="extra-rounded">◻︎</button>
                            <button type="button" class="corners-square-type-btn btn-chip" data-value="dot">●</button>
                            <button type="button" class="corners-square-type-btn btn-chip" data-value="rounded">⬒</button>
                        </div>
                        <label class="flex items-center gap-2 mb-1 text-xs">
                            <input id="cs-grad-enabled" type="checkbox" class="h-3 w-3">
                            <span>Gradient</span>
                        </label>
                        <div id="cs-gradient-options" class="gradient-options hidden">
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                <select id="cs-grad-type" class="p-1 border rounded text-xs">
                                    <option value="linear">Linear</option>
                                    <option value="radial">Radial</option>
                                </select>
                                <input type="color" id="cs-grad-c1" value="#000000" class="w-full h-7 border rounded">
                                <input type="color" id="cs-grad-c2" value="#555555" class="w-full h-7 border rounded">
                                <div class="col-span-2">
                                    <label class="text-xs text-gray-600">Góc (°)</label>
                                    <input id="cs-grad-rot" type="range" min="0" max="360" value="0" class="w-full">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-gray-700 font-semibold text-sm">Eye Ball</label>
                        ...
                        <div id="corners-dot-type-buttons" class="grid grid-cols-6 sm:grid-cols-8 gap-1 mb-1">
                            <button type="button" class="corners-dot-type-btn btn-chip" data-value="square">■</button>
                            <button type="button" class="corners-dot-type-btn btn-chip" data-value="dot">●</button>
                            <button type="button" class="corners-dot-type-btn btn-chip" data-value="rounded">◼︎</button>
                            <button type="button" class="corners-dot-type-btn btn-chip" data-value="classy">◆</button>
                            <button type="button" class="corners-dot-type-btn btn-chip"
                                data-value="classy-rounded">◇</button>
                            <button type="button" class="corners-dot-type-btn btn-chip"
                                data-value="extra-rounded">▣</button>
                        </div>
                        <label class="flex items-center gap-2 mb-1 text-xs">
                            <input id="cd-grad-enabled" type="checkbox" class="h-3 w-3">
                            <span>Gradient</span>
                        </label>
                        <div id="cd-gradient-options" class="gradient-options hidden">
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                <select id="cd-grad-type" class="p-1 border rounded text-xs">
                                    <option value="linear">Linear</option>
                                    <option value="radial">Radial</option>
                                </select>
                                <input type="color" id="cd-grad-c1" value="#000000" class="w-full h-7 border rounded">
                                <input type="color" id="cd-grad-c2" value="#555555" class="w-full h-7 border rounded">
                                <div class="col-span-2">
                                    <label class="text-xs text-gray-600">Góc (°)</label>
                                    <input id="cd-grad-rot" type="range" min="0" max="360" value="0" class="w-full">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-gray-700 font-semibold text-sm">Nền</label>
                            <input type="color" id="background-color" value="#ffffff"
                                class="w-7 h-7 border rounded cursor-pointer">
                        </div>
                        <label class="flex items-center gap-2 mb-1 text-xs">
                            <input id="bg-grad-enabled" type="checkbox" class="h-3 w-3">
                            <span>Gradient</span>
                        </label>
                        <div id="bg-gradient-options" class="gradient-options hidden">
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                <select id="bg-grad-type" class="p-1 border rounded text-xs">
                                    <option value="linear">Linear</option>
                                    <option value="radial">Radial</option>
                                </select>
                                <input type="color" id="bg-grad-c1" value="#ffffff" class="w-full h-7 border rounded">
                                <input type="color" id="bg-grad-c2" value="#f3f4f6" class="w-full h-7 border rounded">
                                <div class="col-span-2">
                                    <label class="text-xs text-gray-600">Góc (°)</label>
                                    <input id="bg-grad-rot" type="range" min="0" max="360" value="0" class="w-full">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {{-- CỘT 2: Tùy chọn QR + Kết quả + Tải về --}}
            <div class="flex flex-col gap-4 min-h-[320px] max-w-xs w-full mx-auto">
                {{-- KHUNG 1: Tùy chọn option --}}
                <div class="p-3 bg-gray-100 rounded-lg border border-gray-200">
                    <h2 class="text-base font-semibold text-gray-800 mb-2">Tùy chọn QR Code</h2>
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="block text-gray-700 font-semibold mb-1 text-sm">Chiều rộng</label>
                            <input id="opt-width" type="number" min="120" max="1024" value="200"
                                class="w-full p-1 border rounded text-sm">
                        </div>
                        <div>
                            <label class="block text-gray-700 font-semibold mb-1 text-sm">Chiều cao</label>
                            <input id="opt-height" type="number" min="120" max="1024" value="200"
                                class="w-full p-1 border rounded text-sm">
                        </div>
                        <div class="col-span-full">
                            <button type="button" id="adv-toggle"
                                ...
                                Tuỳ chọn nâng cao
                                <span id="adv-toggle-icon" class="ml-2 transition-transform">&#9660;</span>
                            </button>
                            <div id="advanced-options" class="space-y-1 hidden">
                                <div>
                                    <label class="block text-gray-700 font-semibold mb-1 text-xs">Kiểu render</label>
                                    <select id="opt-type" class="w-full p-1 border rounded text-xs">
                                        <option value="svg" selected>SVG</option>
                                        <option value="canvas">Canvas</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-gray-700 font-semibold mb-1 text-xs">Error correction</label>
                                    <select id="opt-ecl" class="w-full p-1 border rounded text-xs">
                                ...
                                <div>
                                    <label class="block text-gray-700 font-semibold mb-1 text-xs">TypeNumber (0–40)</label>
                                    <input id="opt-typeNumber" type="number" min="0" max="40" value="0" class="w-full p-1 border rounded text-xs">
                                </div>
                                <div>
                                    <label class="block text-gray-700 font-semibold mb-1 text-xs">Shape</label>
                ...
                {{-- KHUNG 2: Kết quả & tải xuống --}}
                <div class="p-3 bg-gray-100 rounded-lg border border-gray-200 flex flex-col">
                    <h2 class="text-base font-bold text-gray-800 mb-2">Kết quả</h2>
                    ...
                    <div class="mt-3 flex flex-col gap-2">
                        ...
                        <div>
                            <label class="block text-gray-700 font-semibold mb-1 text-sm">Tên file</label>
                            <input id="download-name" value="leminhtriet-qrcode" class="w-full p-1 border rounded text-sm">
                        </div>
                        <div class="grid grid-cols-2 gap-2 items-end">
                            <div>
                                <label class="block text-gray-700 font-semibold mb-1 text-sm">Định dạng tải</label>
...
@section('scripts')
<script src="https://cdn.jsdelivr.net/npm/qr-code-styling@latest/lib/qr-code-styling.min.js"></script>
<script>
